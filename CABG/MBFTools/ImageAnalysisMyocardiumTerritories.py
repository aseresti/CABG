	...
	def __init__(self,Args):
		...
		if self.Args.OutputFileName is None:
			if self.Args.InputFileName.find("/")>=0:
				self.Args.OutputFileName = f"./{self.Args.InputFileName.split('/')[-1].split('.')[0]}_Territories.vtu"
			else:
				self.Args.OutputFileName="MBF_Territories.vtu"
			self.Args.OutputFilename2=self.Args.OutputFileName.replace(".vtu","_Labels.dat")

		#Coronary Centerline Files
		LADFiles        =sorted(glob("%s/L_LAD_*.vtp"%self.Args.VesselSurfaces))
		IntermediusFiles=sorted(glob("%s/L_Intermedius_*.vtp"%self.Args.VesselSurfaces))
		LCxFiles        =sorted(glob("%s/L_LCx_*.vtp"%self.Args.VesselSurfaces))
		Diag1Files      =sorted(glob("%s/L_Diag1_*.vtp"%self.Args.VesselSurfaces))
		Diag2Files      =sorted(glob("%s/L_Diag2_*.vtp"%self.Args.VesselSurfaces))
		PDAFiles        =sorted(glob("%s/R_PDA_*.vtp"%self.Args.VesselSurfaces))
		PLAFiles        =sorted(glob("%s/R_PL_*.vtp"%self.Args.VesselSurfaces))
